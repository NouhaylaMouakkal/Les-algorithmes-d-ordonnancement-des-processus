...
process* choose_PPP_process (processList* l)
{
	...
	for(int i=0; i < l->process_nbr; i++ )
	{
		if(l->allProcess[i].priority < selectedprocess->priority &&
...
void start_PPP_processor(processList* l)
{
	int clock = 0;
	...
	while(is_all_process_end(l) == 0)
	{
		...
		if(runningprocess != choose_PPP_process(l))
		{
			if(runningprocess != NULL)
			{
				if(runningprocess->real_exec_time == runningprocess->exec_time)
				{
					runningprocess = choose_PPP_process (l);
					if(runningprocess != NULL)
